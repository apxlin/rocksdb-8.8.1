...
DEFINE_bool(output_ml_features_windows, false,
            "output txt file with format within time windows\n"
            "query type access count, total unique keys in this cf, average key size, key size median"
            "key size variation, value size average, value size median, value size variation, "
            "mean, mode, median, 1st Quartiles, 3rd Quartiles, skewness, and kurtosis for KV pair accessed");
...
Status TraceAnalyzer::PrepareProcessing() {
  ...
  if (FLAGS_output_ml_features_windows){
    std::string ml_feature_windows_file_name;
    ml_feature_windows_file_name =
        output_path_ + "/ml_feature_windows.txt";
    s = env_->NewWritableFile(ml_feature_windows_file_name, &ml_feature_windows_f_,
                              env_options_);
    if (!s.ok()) {
      return s;
    }
...
Status TraceAnalyzer::EndProcessing() {
  ...
  if (FLAGS_output_ml_features_windows && ml_feature_windows_f_){
    for (int type = 0; type < kTaTypeNum; type++){
      if (!ta_[type].enabled) {
        continue;
      }
      for (auto& stat_it : ta_[type].stats) {
        ...
        auto& time_windows = stat_it.second.time_windows;
        for (const auto& [window_index, window_data] : time_windows) {
          uint64_t a_count = 0;
          uint64_t a_key_size_sum = 0, a_key_size_sqsum = 0;
          uint64_t a_value_size_sum = 0, a_value_size_sqsum = 0;
          std::unordered_map<std::string, uint64_t> key_frequency_map;
          std::vector<uint64_t> key_sizes;
          std::vector<uint64_t> value_sizes;

          for (const auto& trace_u : window_data) {
          uint64_t key_size = trace_u.key.size();
          uint64_t value_size = trace_u.value_size; 
          a_count++;
          a_key_size_sum += key_size;
          a_key_size_sqsum += key_size * key_size;
          a_value_size_sum += value_size;
          a_value_size_sqsum += value_size * value_size;

          key_sizes.push_back(key_size);
          value_sizes.push_back(value_size);

          key_frequency_map[trace_u.key]++;
          }
          // std::cout << "key_sizes: " << key_sizes.size() << std::endl;
          // std::cout << "value_sizes: " << value_sizes.size() << std::endl;
          std::vector<uint64_t> key_access_counts;
          for (const auto& [key, count] : key_frequency_map) {
            key_access_counts.push_back(count);
          }

          double key_size_ave = (a_count > 0) ? (static_cast<double>(a_key_size_sum) / a_count) : 0.0;
          double value_size_ave = (a_count > 0) ? (static_cast<double>(a_value_size_sum) / a_count) : 0.0;
          double key_size_vari = (a_count > 0) ?
            std::sqrt(static_cast<double>(a_key_size_sqsum) / a_count - key_size_ave * key_size_ave) : 0.0;
          double value_size_vari = (a_count > 0) ?
            std::sqrt(static_cast<double>(a_value_size_sqsum) / a_count - value_size_ave * value_size_ave) : 0.0;

          auto calculateMedian = [](std::vector<uint64_t>& sizes) -> uint64_t {
            if (sizes.empty()) return 0;
            std::nth_element(sizes.begin(), sizes.begin() + sizes.size() / 2, sizes.end());
            uint64_t median = sizes[sizes.size() / 2];
            if (sizes.size() % 2 == 0) {
              uint64_t left_middle = *(std::max_element(sizes.begin(), sizes.begin() + sizes.size() / 2));
              median = (median + left_middle) / 2;
            }
            return median;
          };

          uint64_t key_size_median = calculateMedian(key_sizes);
          uint64_t value_size_median = calculateMedian(value_sizes);

          StatisticsCalculator stats(key_access_counts);
          ...
          int unique_keys = key_frequency_map.size();


          int ret;
          ret = snprintf(buffer_, sizeof(buffer_),
                        "%lu,%d,%f,%lu,%f,%f,%lu,%f,%f,%f,%f,%f,%f,%f,%f,\n",
                        a_count, unique_keys, key_size_ave, key_size_median,
                        key_size_vari, value_size_ave, value_size_median, value_size_vari,
                        mean, mode, median, quartiles[0], quartiles[2], skewness, kurtosis);
          if (ret < 0) {
            return Status::IOError("Format the output failed");
          }
          std::string printout(buffer_);
          // std::cout << "buffer: "<< printout << std::endl;
          s = ml_feature_windows_f_->Append(printout);
          if (!s.ok()) {
            fprintf(stderr, "Write access count distribution file failed\n");
            return s;
          }
  ...
  if (ml_feature_windows_f_) {
    s = ml_feature_windows_f_->Close();
  }
...
Status TraceAnalyzer::KeyStatsInsertion(const uint32_t& type,
  ...
  if (FLAGS_output_ml_features_windows) {
    ...
    trace_u.ts = (ts - time_series_start_) / 1000000;
    trace_u.cf_id = cf_id;
    int window_index = trace_u.ts / 10;
    ta_[type].stats[cf_id].time_windows[window_index].push_back(trace_u);
  }
