...
Status DB::Open(const Options& options, const std::string& dbname, DB** dbptr) {
  ...
  // Specify the path to the options file (customize as needed)
  std::string options_file_path = "/home/qi/LLM-Trace-Auto-Tuning/output/options_file.ini";
  ConfigOptions config_opts;
  config_opts.ignore_unknown_options = false;
  config_opts.input_strings_escaped = true;
  config_opts.env = Env::Default();
  // Attempt to load options from the file
  auto load_status = LoadOptionsFromFile(config_opts, options_file_path, &db_opts, &cf_descs);
  ...
  if (!load_status.ok()) {
    ...
    std::cout << "successfully to load options from file" << std::endl;
  }

  std::vector<ColumnFamilyHandle*> handles;
  Status s = DB::Open(db_opts, dbname, cf_descs, &handles, dbptr);

  if (s.ok()){
    assert(handles.size() == 1);
    delete handles[0];
    std::cout << "3. max_subcompaction: " << db_opts.max_subcompactions<< std::endl;
    return s;
  }

  // DBOptions db_options(options);
  // std::cout << "1. max_subcompaction: " << db_options.max_subcompactions<< std::endl;
